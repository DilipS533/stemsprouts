...
export default function TeamCard({name,role,bio,photo,fav,fun,variant}:{name:string;role:string;bio:string;photo?:string;fav?:string;fun?:string;variant?: 'leader'|'mentor'|'default'}){
  const [open, setOpen] = useState(false)
  const initials = name.split(' ').map(n=>n[0]).slice(0,2).join('')
  const isLeader = variant === 'leader'

  return (
    <div className={`team-member ${isLeader? 'p-6' : 'p-4'} ${isLeader? 'card' : 'card-soft'} text-center`}>
      <M.div whileHover={{ scale: 1.02 }} whileFocus={{ scale: 1.02 }} className={`mx-auto ${isLeader? 'w-36 h-36 mb-3' : 'w-32 h-32 mb-3'} rounded-full overflow-hidden bg-gray-700 flex items-center justify-center`}>
        {photo ? <img src={photo} alt={name} className="w-full h-full object-cover"/> : <div className="text-xl font-semibold">{initials}</div>}
      </M.div>
      <h5 className={`mt-1 ${isLeader? 'text-xl' : ''}`} style={{color:'var(--primary)'}}>{name}</h5>
      <div className="role text-sm text-gray-300">{role}</div>

      <div className="mt-3 text-sm text-gray-300">{bio}</div>

      <div className={`mt-4 transition-opacity ${isLeader? 'opacity-100' : 'opacity-0 hover:opacity-100 focus-within:opacity-100'}`}>
          ...
          <M.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative bg-[#071025] max-w-xl w-full rounded-2xl p-6 border border-white/6">
            ...
            <div className="flex gap-4 items-center">
              <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-700 flex items-center justify-center">
                {photo ? <img src={photo} alt={name} className="w-full h-full object-cover"/> : <div className="text-2xl font-semibold">{initials}</div>}
              </div>
              <div>
