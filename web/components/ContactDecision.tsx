  ...
  return (
    <section id="contact" aria-labelledby="contact-heading" className="max-w-6xl mx-auto px-6 py-20">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card title="Volunteer" desc="Lead workshops, mentor students, or help run local events." cta="Sign up" onClick={()=> setInterest('Volunteer')} />
        <Card title="Partner" desc="Host a workshop or collaborate with your school or org." cta="Partner" onClick={()=> setInterest('Partner')} />
        <Card title="Donate" desc="Support materials and local chapters so more students can join." cta="Donate" onClick={()=> setInterest('Donate')} />
        <Card title="Start a chapter" desc="Help start a local STEM Sprouts chapter at your school or community center." cta="Start a chapter" onClick={()=> setInterest('Start a Chapter')} />
      </div>

      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        <form onSubmit={submitMail} className="space-y-3 p-6 rounded-lg bg-[rgba(255,255,255,0.01)] border border-white/6">
          <div className="text-sm text-gray-300">Ready to connect? Fill this out and we'll follow up.</div>
...
function Card({ title, desc, cta, onClick }: { title: string; desc: string; cta: string; onClick: ()=>void }){
  return (
    <button onClick={onClick} className="text-left p-4 rounded-lg border border-white/6 bg-[rgba(255,255,255,0.01)] hover:bg-[rgba(255,255,255,0.02)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]">
